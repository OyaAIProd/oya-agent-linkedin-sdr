...
# Credentials injected by the LinkedIn gateway
        ...
        results.append(
            {
                ...
                "sender_name": (
                    sender.get("name") or sender.get("display_name") or ""
...
def do_send_message(chat_id, text):
    ...
    body = {"account_id": UNIPILE_ACCOUNT_ID, "text": text.strip()}
...
def do_start_chat(attendees_ids, text):
    if not attendees_ids:
        return {"error": "attendees_ids is required (comma-separated LinkedIn user IDs)"}
    if not text or not text.strip():
        return {"error": "text is required"}
...
def run(inp):
    ...
    if not action:
        print(json.dumps({"error": "action is required. Options: list_chats, read_messages, send_message, start_chat, get_chat"}))
        return

    if not UNIPILE_DSN or not UNIPILE_API_KEY or not UNIPILE_ACCOUNT_ID:
        print(json.dumps({"error": "LinkedIn not connected. Please reconnect your LinkedIn account via the gateway."}))
        return

    try:
        if action == "list_chats":
            result = do_list_chats(inp.get("limit", 20))
        elif action == "get_chat":
            result = do_get_chat(inp.get("chat_id", ""))
        elif action == "read_messages":
            result = do_read_messages(inp.get("chat_id", ""), inp.get("limit", 20))
        elif action == "send_message":
            result = do_send_message(inp.get("chat_id", ""), inp.get("text", ""))
        elif action == "start_chat":
            result = do_start_chat(inp.get("attendees_ids", ""), inp.get("text", ""))
        else:
            result = {"error": f"Unknown action '{action}'. Options: list_chats, read_messages, send_message, start_chat, get_chat"}
    except httpx.HTTPStatusError as e:
        result = {"error": f"API error {e.response.status_code}: {e.response.text[:500]}"}
...
# Oya sandbox entrypoint — reads from INPUT_JSON env var (never stdin)
